/*
 * FOX: vendored unmodified from AnvilGUI 1.10.13-SNAPSHOT
 * (https://github.com/WesJD/AnvilGUI, MIT license).
 *
 * Upstream compiles this wrapper at Java 25 (class-file v69). Paper <= 1.21.4 runs
 * its plugin remapper over the whole shaded jar and its bundled ASM rejects v69
 * entries, so the multi-version jar would fail to load there entirely. This module
 * compiles the same source at release 21 (v65), and Plugin/pom.xml filters the
 * upstream v69 classes out of the shade. Remove this copy once upstream publishes
 * the wrapper at a Java 21 bytecode target.
 */
